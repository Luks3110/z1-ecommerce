...
export default class OrderRepository implements IOrderRepository {
    ...
    async create(orderData: CreateOrderData): Promise<ResultTuple<Order & { items: OrderItem[] }>> {
            ...
            return await db.transaction(async (tx) => {
                const result = await tx.execute(sql`
                    WITH product_validation AS (
                        ...
                        FROM (
                            SELECT 
                                unnest(ARRAY[${sql.join(orderData.items.map(item => item.productId), sql`,`)}]::int[]) as product_id,
                                unnest(ARRAY[${sql.join(orderData.items.map(item => item.quantity), sql`,`)}]::int[]) as quantity
                        ) q
                ...
                const { items, ...order } = result.rows[0] as unknown as Order & { items: OrderItem[] };
    ...
    async findById(id: number): Promise<ResultTuple<Order & { items: OrderItem[] }>> {
        try {
            const [order] = await this.neon.drizzle()
                .select()
                .from(orders)
            ...
            const foundOrderItems = await this.neon.drizzle()
                .select()
                .from(orderItems)
    ...
    async findAll(): Promise<ResultTuple<Array<Order & { items: OrderItem[] }>>> {
        try {
            ...
            const allOrderItems = await this.neon.drizzle()
                .select()
                .from(orderItems)
                .where(inArray(
                    orderItems.orderId, 
    ...
    async findByUserId(userId: number): Promise<ResultTuple<Array<Order & { items: OrderItem[] }>>> {
        try {
            const userOrders = await this.neon.drizzle()
                .select()
                .from(orders)
            ...
            const itemsByOrder = foundOrderItems.reduce((acc, item) => {
                if (!acc[item.orderId]) {
                    acc[item.orderId] = [];
                }
                acc[item.orderId].push(item);
